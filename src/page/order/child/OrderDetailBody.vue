    ...
    <div class="order-body" v-if="orderDetail && orderDetail.order" v-bind:class="{'ship': orderDetail.order.status == 1}">
      ...
      <div class="address">
        <div>
          ...
          <span class="mobile" v-bind:style="-webkit-box-orient: vertical; -webkit-line-clamp: 2;">{{orderDetail.order.consignee.mobile}}</span> 
        </div>
...
import {
  ...
  orderConfirm,
  orderRebuy
} from "../../../api/network/order"; //订单详情 //获取退货原因 //取消订单 //再次购买
